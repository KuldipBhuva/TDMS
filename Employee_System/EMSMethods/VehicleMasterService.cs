    ...
    public class VehicleMasterService
    {
        ...
        public List<VehicleItem> VehicleListData(string status, int cid)
        {
            var objData = (from vehicle in DbContext.VehicleMasters
                           ...
                           join br in DbContext.Branch_master on comp.id equals br.id
        ...
        public List<VehicleTypeModel> getActiveVType()
        {
            ...
            List<VehicleTypeMaster> lstBranch = DbContext.VehicleTypeMasters.Where(m=>m.Status==true).ToList();
            List<VehicleTypeModel> lstBranchItem = Mapper.Map<List<VehicleTypeModel>>(lstBranch);
            return lstBranchItem;
        }
    }
}
