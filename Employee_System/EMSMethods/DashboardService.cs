...
namespace EMSMethods
{
    public class DashboardService
    {
        ...
        public List<TripModel> getTmrwTrips(int cid, int bid)
        {
            DateTime today = System.DateTime.Today;
            DateTime tmrw = System.DateTime.Today.AddDays(1);
            Mapper.CreateMap<TripMaster, TripModel>();
            List<TripMaster> objCompanyMaster = DbContext.TripMasters.Where(m => m.OIType == "O" && m.CID == cid && m.BID == (bid == 0 ? m.BID : bid) && m.Status == 1 && EntityFunctions.TruncateTime(m.DDtTime.Value) == tmrw).OrderBy(m => m.DDtTime).ToList();
            List<TripModel> objCompItem = Mapper.Map<List<TripModel>>(objCompanyMaster);
            return objCompItem;

        }
        public List<TripModel> getnext5Trips(int cid, int bid)
        {
            DateTime today = DateTime.Today;
            DateTime Next5 = today.AddDays(+5);
            DateTime tmrw = System.DateTime.Today.AddDays(1);
            Mapper.CreateMap<TripMaster, TripModel>();
            List<TripMaster> objCompanyMaster = DbContext.TripMasters.Where(m => m.OIType == "O" && m.CID == cid && m.BID == (bid == 0 ? m.BID : bid) && m.Status == 1 && EntityFunctions.TruncateTime(m.DDtTime.Value) > today && EntityFunctions.TruncateTime(m.DDtTime.Value) <= Next5).OrderBy(m => m.DDtTime).ToList();
            List<TripModel> objCompItem = Mapper.Map<List<TripModel>>(objCompanyMaster);
            return objCompItem;

        }
        public List<TripModel> getTodayTrips(int cid, int bid)
        {
            DateTime today = System.DateTime.Today;
            DateTime tmrw = System.DateTime.Today.AddDays(1);
            Mapper.CreateMap<TripMaster, TripModel>();
            List<TripMaster> objCompanyMaster = DbContext.TripMasters.Where(m => m.OIType == "O" && m.CID == cid && m.BID == (bid == 0 ? m.BID : bid) && m.Status == 1 && EntityFunctions.TruncateTime(m.DDtTime.Value)>=today && EntityFunctions.TruncateTime(m.RDtTime.Value) <= today).OrderBy(m => m.DDtTime).ToList();
            List<TripModel> objCompItem = Mapper.Map<List<TripModel>>(objCompanyMaster);
            return objCompItem;
